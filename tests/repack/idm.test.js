...
import { fint, organizationFixed } from "../../config.js"
...
import { createAditroUnits, createTestOrgUnit, getValidExcepttionRules } from "./test-org.js"
...
describe("repackFintIdmEnheter works as expected when", () => {
  ...
  it("test org is repacked, but has validation errors when no exceptions are defined", () => {
    ...
    const correspondingBottomUnitMissing = expectedNoBottomUnits.every((expectedNoBottomUnit) =>
      repackResult.tests.correspondingBottomUnitMissing.data.some((unit) => unit.organisasjonsId.identifikatorverdi === expectedNoBottomUnit)
    )
    ...
    const differentNameChain = expectedDifferentNameChain.every((expectedDifferentNameChainUnit) =>
      repackResult.tests.correspondingBottomUnitDifferentName.data.some((unit) => unit.organisasjonsId.identifikatorverdi === expectedDifferentNameChainUnit)
    )
...
describe("repackFintIdmEnheter works as expected when", () => {
  ...
  it("We add extra exception rules and get valid result", () => {
    ...
    const inheritedGrandchildren = absorbChildrenOverrideResult._links.underordnet.filter((child) =>
      [
        `${url}/administrasjon/organisasjon/organisasjonselement/organisasjonsid/annen-drit-1120`,
        `${url}/administrasjon/organisasjon/organisasjonselement/organisasjonsid/annen-drit-1121`,
        `${url}/administrasjon/organisasjon/organisasjonselement/organisasjonsid/test-fellesvgs-11100`
      ].includes(child.href)
    )
    assert.strictEqual(inheritedGrandchildren.length, 3)

    const absorbedChildren = repackResult.resultingUnitsFlat.filter(
      (unit) =>
        ["annen-drit-112", "test-fellesvgs-111", "test-fellesvgs-1111", "test-fellesvgs-11110"].includes(unit.organisasjonsId.identifikatorverdi) ||
        ["annen-drit-112", "test-fellesvgs-111", "test-fellesvgs-1111", "test-fellesvgs-11110"].includes(unit.abstractEnhetOrganisasjonsId)
    )
    ...
    assert.strictEqual(
      repackResult.resultingUnitsFlat.some((unit) => typeof unit.organisasjonsType.navn !== "string" || unit.organisasjonsType.navn.length < 1),
      false
    )
  ...
  it("We mess with aditro units and get invalid result", () => {
    ...
    assert.strictEqual(
      repackResult.tests.missingAditroOrgType.data.some((unit) => unit.organisasjonsId === "test-organisasjon-13000"),
      true
    )
    assert.strictEqual(
      repackResult.tests.missingAditroOrgType.data.some((unit) => unit.organisasjonsId === "test-fellesvgs-11110"),
      true
    )
    assert.strictEqual(
      repackResult.tests.missingAditroOrgType.data.some((unit) => unit.organisasjonsId === "test-digi-13010"),
      true
    )
