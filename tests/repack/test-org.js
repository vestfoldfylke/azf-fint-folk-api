...
const createTestOrgUnit = (unit) => {
  if (!unit || typeof unit !== "object") {
    throw new Error('Missing required parameter object "unit"')
  }
  ...
  if (!overordnetId) {
    throw new Error('Missing required parameter "overordnetId"')
  }
  if (gyldighetsperiode && !gyldighetsperiode.start) {
...
const createAditroUnits = (units) => {
  if (!units || !Array.isArray(units)) {
    throw new Error('Missing required parameter "units" as an array')
  }
...
export { createAditroUnits, createTestOrgUnit, getValidExcepttionRules }
