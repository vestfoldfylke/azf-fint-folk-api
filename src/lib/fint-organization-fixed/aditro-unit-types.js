...
const getAditroOrgUnits = async () => {
  ...
  if (!response.ok) {
    ...
    logger.error(`Failed to fetch aditro org units, status: ${response.status}, statusText: ${response.statusText}, errorData: {@errorData}`, errorData)
    throw new Error(`Failed to fetch aditro org units, status: ${response.status}, statusText: ${response.statusText}`)
  }
...
export { getAditroOrgUnits, getAditroProjectDimension6Hours }
