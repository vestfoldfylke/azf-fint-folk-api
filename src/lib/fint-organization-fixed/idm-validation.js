...
const validateRawOrganizationUnits = (rawOrganizationUnits) => {
  const validationResult = {
    ...
    tests: {
      ...
      brokenChildRelation: {
        description:
          "Enheter som har underordnet som ikke finnes (eller har duplikat), eller der en underordnet enhet ikke peker tilbake på overordnet. OBS! Vi tillater disse, da vi ikke tillater ødelagte parent-relasjoner - men de kan være nyttige å vite om, da enheten blir ekskludert fra resultatet.",
        data: [],
        status: "not run"
      },
  ...
  try {
    ...
    validationResult.tests.allowedNumberOfRawUnits.status =
      organizationFixed.idmMinimumUnits <= validationResult.tests.allowedNumberOfRawUnits.data && validationResult.tests.allowedNumberOfRawUnits.data <= organizationFixed.idmMaximumUnits
        ? "pass"
        : "fail"
    ...
    validationResult.tests.abstractWithArbeidsforhold.data = enheter.filter(
      (enhet) => Array.isArray(enhet._links.arbeidsforhold) && enhet._links.arbeidsforhold.length > 0 && Array.isArray(enhet._links.underordnet) && enhet._links.underordnet.length > 0
    )
    ...
    validationResult.tests.allowedNumberOfValidUnits.status =
      organizationFixed.idmMinimumUnits <= validationResult.tests.allowedNumberOfValidUnits.data && validationResult.tests.allowedNumberOfValidUnits.data <= organizationFixed.idmMaximumUnits
        ? "pass"
        : "fail"
...
const validateExceptionRules = (exceptionRules, preValidatedUnits) => {
  ...
  try {
    ...
    for (const [key, value] of Object.entries(overrideNextProbableLink)) {
      ...
      if (!nextLinkIsChild) {
        const invalidRule = generateInvalidRule(
          exceptionRuleName,
          key,
          value,
          `Next link "${value.nextLink.href}" er ikke en underenhet av enheten - enten feil oppsett, eller HR har endret noe... sjekk rådata :(`
        )
        validationResult.invalidRules.push(invalidRule)
      }
    }
      ...
      for (const absorbChild of value.absorbChildren) {
        ...
        if (!nextLinkIsChild) {
          const invalidRule = generateInvalidRule(
            ...
            key,
            value,
            `Absorb child link "${absorbChild.href}" er ikke en underenhet av enheten - enten feil oppsett, eller HR har endret noe... sjekk rådata :(`
          )
          validationResult.invalidRules.push(invalidRule)
          break // OBS her breaker vi, for å ikke legge til den samme regelen flere ganger
        }
    ...
    for (const [key, value] of Object.entries(manualLeaders)) {
      ...
      if (!value.leader.href.startsWith(`${url}/administrasjon/personal/personalressurs/ansattnummer/`)) {
        const invalidRule = generateInvalidRule(
          ...
          key,
          value,
          `leader.href-property starter ikke med ${url}/administrasjon/personal/personalressurs/ansattnummer/ - sett opp regelen korrekt...`
        )
        validationResult.invalidRules.push(invalidRule)
        continue
      }
    ...
    logger.error("Internal error when validating exception-rules {err}", error.stack || error.toString())
    validationResult.internalError = error.stack || error.toString()
    validationResult.valid = false
    return validationResult
  }
...
export { getUnitFromLink, validateExceptionRules, validateRawOrganizationUnits }
