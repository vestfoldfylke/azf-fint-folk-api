...
const getFixedUnits = async () => {
  ...
  if (cachedOrgNested && cachedOrgFlat && cachedGraphQlFlat) {
    logger.info("getFixedUnits - found valid values in cache, will use that instead of fetching new")
  ...
  const { repackedFintUnitsResult } = await fintOrganizationFixedIdm()

  if (!repackedFintUnitsResult) {
    throw new Error("fintOrganizationFixedIdm failed - cannot return fixed units - check validate")
  }
  if (!repackedFintUnitsResult.valid) {
    throw new Error("fintOrganizationFixedIdm result was not valid - cannot return fixed units - check validate")
  }
