...
import { aditro, organizationFixed } from "../../../config.js"
import { fintRest } from "../requests/call-fint.js"
import { getExceptionRules } from "./exception-rules.js"
import { getUnitFromLink, validateExceptionRules, validateRawOrganizationUnits } from "./idm-validation.js"

const { maxUnitsWithUnknownOrgType } = aditro

import { getAditroOrgUnits, getAditroProjectDimension6Hours } from "./aditro-unit-types.js"
...
const repackFintIdmEnheter = (topUnits, validatedUnits, aditroUnits, exceptionRules) => {
  ...
  const repackFintEnhetRecursive = (unit) => {
    ...
    while (Array.isArray(correspondingBottomUnit._links.underordnet) && correspondingBottomUnit._links.underordnet.length > 0) {
      // As long as there are underordnet, we keep going down until bottom level
      ...
      if (useAbstractAsUnit) {
        logger.warn(`UseAbstractAsUnit rule for ${unitCopy.navn} (${unitCopy.organisasjonsId.identifikatorverdi}) - using abstract unit as actual unit`)
        validation.handledByExceptionRules.push({
          rule: "useAbstractAsUnitOverride",
          data: { organisasjonsId: unitCopy.organisasjonsId.identifikatorverdi, navn: unitCopy.navn },
          description: "Brukte abstrakt enhet istedet for å lete nedover"
        })
      ...
      if (nextLinkOverride) {
        logger.info(
          `Override rule for ${correspondingBottomUnit.navn} (${correspondingBottomUnit.organisasjonsId.identifikatorverdi}) - overriding nextProbableLink to ${nextLinkOverride.nextLink.href}`
        )
        validation.handledByExceptionRules.push({
          rule: "overrideNextProbableLink",
          data: { organisasjonsId: correspondingBottomUnit.organisasjonsId.identifikatorverdi, navn: correspondingBottomUnit.navn },
          description: `Overstyrte normal regel og gikk direkte videre til ${nextLinkOverride.nextLink.href} (${nextLinkOverride.nextLink.navn}) i stedet for å legge til "0" eller "00" på id`
        })
    ...
    abstractTraversedUnits.unshift({
      organisasjonsId: unitCopy.organisasjonsId.identifikatorverdi,
      navn: unitCopy.navn,
      href: unitCopy._links.self.find((link) => link.href.includes("organisasjonsid")).href
    })
    ...
    if (abstractTraversedUnitsNameChain.some((name) => name.toLowerCase() !== correspondingBottomUnit.navn.toLowerCase())) {
      ...
      if (!vgsNameChain(abstractTraversedUnitsNameChain)) {
        // If not vgs special case, we have to check more
        ...
        if (nameChainOverrideRule && nameChainOverrideRule.allowedNameChain.join(",").toLowerCase() === abstractTraversedUnitsNameChain.join(",").toLowerCase()) {
          logger.warn(`NameChainOverride rule for ${unitCopy.navn} (${unitCopy.organisasjonsId.identifikatorverdi}) - allowing different name chain`)
          validation.handledByExceptionRules.push({
            rule: "nameChainOverride",
            data: { organisasjonsId: unitCopy.organisasjonsId.identifikatorverdi, navn: unitCopy.navn, allowedNameChain: nameChainOverrideRule.allowedNameChain },
            description: "Overstyrte navneregler og tillot annen navnekjede"
          })
        } else {
    ...
    if (Array.isArray(allUnderordnet)) {
      // Then we recursively handle all underordnet (except the abstract itself units) the same way
      ...
      if (absorbChildrenOverride) {
        logger.info(`absorbChildOverrides for ${unitCopy.navn} - absorbing ${absorbChildrenOverride.absorbChildren.length} children`)
        ...
        grandChildrenToAddToCurrent = childrenToAbsorb.flatMap((child) => child.underordnet)
        validation.handledByExceptionRules.push({
          rule: "absorbChildrenOverrides",
          data: {
            organisasjonsId: unitCopy.organisasjonsId.identifikatorverdi,
            navn: unitCopy.navn,
            absorbedChildren: childrenToAbsorb.map((child) => ({ organisasjonsId: child.organisasjonsId, navn: child.navn }))
          },
          description: "Absorberte barn - barnet blir borte, men parent overtok barnets underordnede"
        })
      }
      ...
      logger.warn("No underordnet for unit, dont think this should happen... {navn}", unitCopy.navn)
    }
    ...
    if (aditroProjectDimension6HoursResult.unitNotFound) {
      logger.warn(`No aditro unit found for unit ${correspondingBottomUnit.navn} (${correspondingBottomUnit.organisasjonsId.identifikatorverdi}), dont think this should happen...`)
    ...
    if (aditroProjectDimension6HoursResult.missingProjectDimension6Hours || aditroProjectDimension6HoursResult.invalidProjectDimension6Hours) {
      logger.info(`No projectDimension6Hours found or invalid for unit ${correspondingBottomUnit.navn} (${correspondingBottomUnit.organisasjonsId.identifikatorverdi}), using 'ukjent'`)
    ...
    if (manualLeader) {
      logger.warn(`ManualLeader rule for ${unitCopy.navn} (${unitCopy.organisasjonsId.identifikatorverdi}) - adding/replacing with manual leader ${manualLeader.leader.href}`)
      repackedFintUnit._links.leder = [manualLeader.leader]
      actualUnit._links.leder = [manualLeader.leader]
      validation.handledByExceptionRules.push({
        rule: "manualLeaders",
        data: { organisasjonsId: unitCopy.organisasjonsId.identifikatorverdi, navn: unitCopy.navn, manualLeader: manualLeader.leader },
        description: "Overstyrte leder og la til manuell fra exception-rule"
      })
    }
    ...
    if (!willBeAbsorbed) {
      ...
      logger.info(`Unit ${unitCopy.navn} (${unitCopy.organisasjonsId.identifikatorverdi}) will be absorbed by parent ${willBeAbsorbed[0]} - not adding to resultingUnitsFlat`)
    }
    ...
    if (handledUnits.some((handledUnit) => handledUnit.organisasjonsId === unitCopy.organisasjonsId.identifikatorverdi)) {
      logger.warn(`Unit ${unitCopy.navn} (${unitCopy.organisasjonsId.identifikatorverdi}) has already been handled - this should not happen...`)
  ...
  try {
    ...
    validation.tests.allowedNumberOfRepackedUnits.status =
      resultingUnitsFlat.length >= organizationFixed.idmMinimumUnits && resultingUnitsFlat.length <= organizationFixed.idmMaximumUnits ? "pass" : "fail"
    ...
    validation.tests.missingAditroOrgType.status =
      validation.tests.missingAditroOrgType.data.length === 0 ? "pass" : validation.tests.missingAditroOrgType.data.length < maxUnitsWithUnknownOrgType ? "warn" : "fail"
...
const fintOrganizationFixedIdm = async () => {
  ...
  const repackedFintUnitsResult = repackFintIdmEnheter(rawValidationResult.tests.topUnits.data, rawValidationResult.validUnits, aditroUnits, exceptionRules)

  return { rawValidationResult, exceptionRuleValidationResult, repackedFintUnitsResult }
}

export { fintOrganizationFixedIdm, repackFintIdmEnheter, vgsNameChain }
