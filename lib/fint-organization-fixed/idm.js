...
import { validateRawOrganizationUnits, validateExceptionRules, getUnitFromLink } from './idm-validation.js'
import { getExceptionRules } from './exception-rules.js'
import { organizationFixed, aditro } from '../../config.js'
...
const repackFintIdmEnheter = (topUnits, validatedUnits, aditroUnits, exceptionRules) => {
  ...
  const repackFintEnhetRecursive = (unit) => {
    ...
    while (Array.isArray(correspondingBottomUnit._links.underordnet) && correspondingBottomUnit._links.underordnet.length > 0) { // As long as there are underordnet, we keep going down until bottom level
      ...
      if (nextLinkOverride) {
        logger.info(`Override rule for ${correspondingBottomUnit.navn} (${correspondingBottomUnit.organisasjonsId.identifikatorverdi}) - overriding nextProbableLink to ${nextLinkOverride.nextLink.href}`)
