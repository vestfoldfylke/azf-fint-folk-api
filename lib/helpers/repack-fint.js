 ...
 * @property {string?} beskrivelse - Et beskrivende navn på perioden
 * @property {string?} start - UTC-string startdato for perioden
 * @property {string?} slutt - Sluttdato for perioden eller ikke noe verdi i det hele tatt
 */

/**
 * @typedef {Object} RepackedFintPeriode
 * @property {string?} beskrivelse - Et beskrivende navn på perioden
 * @property {string?} start - UTC-string startdato for perioden ALWAYS at 00:00:00
 * @property {string?} slutt - UTC-string sluttdato for perioden ALWAYS at 23:59:59 eller null
 * @property {string?} fintStart - UTC-string startdato som ble returnert fra FINT
 * @property {string?} fintSlutt - UTC-string sluttdato som ble returnert fra FINT
 * @property {boolean} aktiv - Om perioden er aktiv eller ikke
 */
 ...
 *
 * @param {FintPeriode} periode
 * @returns {RepackedFintPeriode?}
 */
const repackPeriode = (periode) => {
  if (!periode) {
    return {
      beskrivelse: null,
      start: null,
      ...
      fintStart: null,
      fintSlutt: null,
      aktiv: true // Anta at ingen periode er aktiv av en eller annen grunn...
    }
  }
  const stupidPeriodWithActualTimestamps = {
    beskrivelse: periode.beskrivelse || null,
    start: periode.start && isValidDate(periode.start) ? `${new Date(periode.start).toISOString().substring(0, 11)}00:00:00.000Z` : null, // Haha, jag er dum
    slutt: periode.slutt && isValidDate(periode.slutt) ? `${new Date(periode.slutt).toISOString().substring(0, 11)}23:59:59.999Z` : null
  }
  return {
    ...stupidPeriodWithActualTimestamps,
    fintStart: periode.start || null,
    fintSlutt: periode.slutt || null,
    aktiv: aktivPeriode(stupidPeriodWithActualTimestamps)
  }
}
...
const repackSkolear = (skolear) => {
  return {
    ...skolear,
    gyldighetsperiode: repackPeriode(skolear.gyldighetsperiode)
  }
}
...
module.exports = { aktivPeriode, repackPeriode, skoleElementIsAktiv, repackTermin, repackSkolear, repackNavn, repackAdresselinje, createStruktur, repackSkole, repackMiniSkole, getAge, getNarmesteLeder, repackLeder }
